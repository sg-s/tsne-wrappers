function [yData,betas,P,errors] = fit_d(D, parameters)
...
D = D / max(D(:));                                                      
[P,betas] = TSNE.berman.d2p_sparse(D .^ 2, perplexity, sigmaTolerance);                                     
P(isnan(P)) = 0;
...
% Run t-SNE
if initial_solution
    [yData,errors] = TSNE.berman.fit_p_sparse(P, parameters, yData, relTol);
else
    [yData,errors] = TSNE.berman.fit_p_sparse(P, parameters, no_dims, relTol);
end
